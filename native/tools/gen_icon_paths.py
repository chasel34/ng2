...
RN 版(src/ui/icon.tsx)把 Material Icons Outlined 的字形当文字画;原生这边不引字体资源,
而是把**同一颗字形的轮廓**导出成路径常量,由 ui/icons/AppIcons.kt 填充绘制——
形状因此与 RN 逐点一致,又不多一份 331KB 的字体和字形回退风险。

跑法(需要 fontTools):
    uv run --with fonttools python native/tools/gen_icon_paths.py
"""
...
RN_GLYPHS = ROOT / "src/ui/icons.generated.ts"
OUT = ROOT / "native/app/src/main/kotlin/com/chasel/ng2n/ui/icons/IconPaths.generated.kt"
...
def rn_names() -> list[str]:
    src = RN_GLYPHS.read_text(encoding="utf-8")
    return re.findall(r"^  ([a-z_0-9]+): '\\u", src, re.M)
...
def main() -> int:
    ...
    names = rn_names()
    assert names, "没从 src/ui/icons.generated.ts 里扫到图标名"
    for extra in EXTRA:
    ...
    lines = [
        "// 本文件由 native/tools/gen_icon_paths.py 生成,请勿手改。",
        "// 轮廓取自 assets/fonts/MaterialIconsOutlined-Regular.otf(Apache-2.0),",
        "// 与 RN 侧 src/ui/icons.generated.ts 同名同码点的那一颗字形逐点一致。",
        "",
