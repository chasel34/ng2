...
Kotlin / Jetpack Compose 项目，安卓工程位于仓库根目录。Expo / React Native 代码已移除；历史实现可通过 Git 历史查看。

正式包名为 `com.chasel.ng2`，开发包名为 `com.chasel.ng2.dev`。

## 环境

JDK 17、`ANDROID_HOME` 指向本机 SDK。Gradle wrapper 9.7.1(首次会自己下)。
**所有 gradle 命令在仓库根目录下跑,并且要把代理显式传给 JVM**——gradle 是 JVM 进程,
