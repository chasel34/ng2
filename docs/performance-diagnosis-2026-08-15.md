...
> 历史记录：本文针对 2026-08 的 RN/Expo 实现，保留为性能判据和原生重写决策的证据。文中的 TS/TSX 文件、插件、包名与测量值属于当时版本，不是当前 Kotlin 工程的待修复清单。现行测量规则见 [性能手册](perf-playbook.md)，当前包名和构建方式见 [README](../README.md)。
