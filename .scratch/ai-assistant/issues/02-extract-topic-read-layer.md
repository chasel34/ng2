...
**What to build:** 主题页的原始读取（含缓存、按 pid 定位楼层、已加载页查询、热门回复）从 ui/topic 移到 data 层，成为 UI 与后续 AI 工具共同使用的读取入口。主题页面行为、缓存淘汰规则与 READ 反封锁链不变。渲染相关逻辑留在 ui。这是为票 04/05 做的预备重构，不引入任何 AI 代码。

**Blocked by:** None — can start immediately

**Status:** implemented

- [x] 主题详情、回复链、书签预览等现有调用方改为经 data 层读取，页面行为无变化
- [x] data 层不依赖 ui 包；core 层保持纯 Kotlin
- [x] 现有单测通过；补充读取层单测覆盖：按 pid 定位楼层、已加载页命中缓存、热门回复获取
- [x] 主题缓存表结构与淘汰规则不变，无需 Room 迁移
