...
**What to build:** 首次概览与快捷操作通过 Koog 的 skills 机制运行：随包携带的 SKILL.md 与参考文件在启动或版本变化时释放到带版本号的应用私有目录，用 Koog 的 skills 发现生成目录 prompt，模型用框架的目录与文件读取工具按需读取正文。首期 skills：讨论概览、事实核查、批判性思考、梳理分歧、补充背景、查找前情、检查各方论证，以及个人分析的三项（倾向依据、相反表述、观点变化，正文由票 11 完善）。界面上：输入 `/` 或点按钮弹出快捷操作菜单，支持输入筛选、键盘上下与 Enter/Tab 选择、Esc 关闭、空结果提示；面板底部按入口显示快捷胶囊，生成中禁用；快捷操作发出的用户气泡带闪电图标；回答完成后显示「继续问」建议问题。本票只接主题详情与楼层两个入口的快捷操作；事实核查在没有搜索工具时须在回答中说明外部核查未做。

**Blocked by:** 05 论坛读取工具与工具调用行

**Status:** implemented

- [x] 应用升级后 skills 目录按新版本重新释放，旧版本目录清理；对话记录当时激活的 skills 版本
- [x] 模型请求中 skills 目录只含名称与用途，正文只在模型读取后进入上下文（离线单测断言首个请求体）
- [ ] 真机：`/` 菜单筛选、键盘选择、空态文案与设计稿一致；点快捷操作后在当前对话继续
- [x] 每个内置 skill 有一份回答示例记录在票内 Comments，供体验评审
- [x] 不注册脚本执行工具；若框架要求，占位工具固定拒绝并有单测
