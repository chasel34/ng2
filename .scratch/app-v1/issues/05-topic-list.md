...
- `anonymous.ts` —— 匿名还原。**算法与两张字符表直接取自 NGA 官方前端 `commonui.anonyName`**
  ...
  **取证位置:`js_commonui.js` 而不是 06 票用的 `js_bbscode_core.js`**(后者搜 anony 零命中);
  同目录下 `js_read.js` 只有调用点。以后再找官方算法,别只翻 bbscode_core。
  另有一路独立对拍:真实响应里的 `#anony_d43225f5…` 解出「卯邱潘巳邵卢」,而设计稿
  `NGA客户端.dc.html:1124` 的 mock 里有个作者就叫「卯邱潘」——那是设计师照真机同一版块转录的,
  且**早于本实现**随初始提交进仓库。一边官方算法、一边官方 App 渲染结果,两边对上了。
