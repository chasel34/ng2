...
- 十场景:10/10 完成;**全部通过**(场景 9 按票 58 终裁口径通过,见文末「终裁」)。
...
| 9 | 各转场 latch2present / 连续丢帧 | **通过(改判口径)** | present cadence 中位 8.319ms、无连续丢 >2 vsync;双峰占比(终轮 87.9%)降为观测项,依据见票 58 终裁;抽屉 GPU 余量以票 59(P2)延续 |
| 10 | 动画交互伞条款 | **通过** | 附件、画廊修复复验通过，其余交互首轮均通过 |

## 结论

十场景全部完成:**10/10 通过,票 19 性能总闸通过,票 19 resolved**。
场景 9 按票 58 终裁(2026-08-27)改判口径通过——present cadence 中位 8.319ms、
无连续丢 >2 vsync;`SF start→present` 双峰占比降为观测项(四份同口径 trace 为
32.3%/55.1%/74.2%/87.9%,与 app 行为不相关,系会话级随机粘滞的系统侧队列深度,
`--by-peak` 实测 high/low GPU p50 差 3.494ms 证实测量地板)。十场景外追加的票 57
三轮专项终轮已通过。抽屉 GPU 光栅余量(low-peak 口径 p95<6ms 的实证)由票 59 以
P2 延续,不阻塞本票。
