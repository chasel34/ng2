...
**主控裁定(2026-08-23)**:合并。看图页内距保留 16(RN 的 32 是内距加了两遍的 bug,按「已知缺陷不随迁」不照抄);其余按 Expo 对齐。待真机/模拟器重截复验。
...
| 量 | Expo | 原生(修后) | 原生(修前) |
|---|---|---|---|
| 页面底(图外) | `#fcf4e1` | **`#fcf4e1`** | 纯黑 |
| 顶栏底 | `#14796b` | **`#14796b`** | 黑 |
| 状态栏底 | `#14796b` | **`#14796b`** | 黑 |
| 顶栏下沿 y | 274px | **274px** | 浮层式,无下沿 |
| 状态栏图标 | 白 | 白 | 白 |
| 保存图标 | 软盘 | 软盘(票 40) | 下载箭头 |

**图片显示宽(裁定项)**:Expo 912px = 屏宽 84.4%(= 1080 − 2×32dp);
原生 **996px = 92.2%**(= 1080 − 2×16dp),**比 Expo 宽 9.2%**。
这正是「主控裁定:内距保留 16,不照抄 RN 那份加了两遍的 32」的预期结果,
按裁定判**通过**;两边图的长宽比同为 1.206,说明都是宽度受限的 fit,只有内距一档不同。

对照图 [`../acceptance/visual/after/20.png`](../acceptance/visual/after/20.png)
