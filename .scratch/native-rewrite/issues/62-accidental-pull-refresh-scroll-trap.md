...
**Status:** resolved(方向门修复;9-1 复验补记见文末)
...
用户复报「滑不动」,经确定性脚本复现定位为**票 63**(行横划判据局部坐标误认领,
单把长拖中途吃掉手势),与本票机制无关。本票口径的部分复验:20 把连续慢滑
(纯纵向)期间列表逐把正常推进、未出现截断/拽回顶部;回勾专项(复验第 1 条的
`input motionevent` 上滑+回勾 20 把)仍待做。
