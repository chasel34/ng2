...
**通过 —— 复验站在修票的结论这一边:原生不是偏大,是略窄,且没有改过任何字号。**

这一轮两边打开的是**同一个版块 fid=459(守望先锋)**,并且屏上有**同一条主题**,
可以逐字对拍(全分辨率,非半缩放):

| 量 | Expo | 原生 | 差 |
|---|---|---|---|
| 「[赛事杂谈]Insane LFT」深色墨迹宽 | x 45–500,**456px** | x 45–490,**446px** | 原生 **−2.2%** |
| 「队友招募」chip 外框 | x 37–241,**205px = 78dp** | x 37–232,**196px = 74.7dp** | 原生 **−4.4%** |

票面写的 +6.5% / +11% 一条都没复现,方向还反了。两处 chip 的**左沿都是 x=37**,
说明左内距同位,差的只是 CJK 字形宽——同一现象在票 41 的四条指示条(−1.5~2.3%)、
票 39 的顶栏标题墨迹上都出现,是 Compose 与 RN 文本测量的系统性差,不是本屏的档位问题。

「同一屏 Expo 8 条、原生 7 条」也没复现:两边各 8 条。

对照图 [`../acceptance/visual/after/03.png`](../acceptance/visual/after/03.png)
