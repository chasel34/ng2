  ...
  return new QueryClient({
    defaultOptions: {
      queries: {
        ...
        /**
         * 全局保持「立刻过期」,新鲜度**按接口各自定**(详情页在
         * `store/topic-detail.ts` 的 `TOPIC_DETAIL_STALE_MS`)。
         *
         * 一刀切一个全局值在这个 app 里是错的:各接口的「多久算旧」差着量级——
         * 一页楼层几分钟不变都正常,而版块主题列表、通知、签到状态正是用户
         * 进屏就想看到最新的那种。全局设值等于悄悄给所有接口都上了缓存,
         * 出问题时还很难看出是缓存吃掉了请求(排查 P0「版块出不来内容」那种
         * 现象时尤其致命——请求根本没发,日志里什么都看不到)。
         *
         * 写成显式的 0 而不是省略:这是个决定,不是没想过。
         */
        staleTime: 0,
      },
    },
