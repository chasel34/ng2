 ...
 *
 * **动画一律走 Reanimated,不用 RN 自带 `Animated`**(CLAUDE.md「动画」节):
 * RN `Animated.timing` 在 JS 侧把缓动曲线预采样成 **60fps 关键帧数组**,原生驱动
 * 按 16.67ms 桶取值不插值,120Hz 屏上每两个 vsync 才前进一步——2026-08-15 真机
 * 录屏逐帧实测,抽屉面板位移与遮罩透明度每个值精确重复两帧,体感"不连贯"。
 * Reanimated 的 `withTiming` 每个 vsync 在 UI 线程现算曲线,才是真 120Hz。
 * 所以这里只提供 Reanimated 的 `EasingFunctionFactory`,不再导出 RN 版缓动。
 */
