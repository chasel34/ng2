 ...
 * pointer = (lead - 0x81) * 190 + (trail < 0x7F ? trail - 0x40 : trail - 0x41)，
 * 直接用 charCodeAt(pointer) 取码点（全部 23940 个 pointer 都有映射，无空洞）。
 * 每行 190 个字符 = 一个 lead byte（第 0 行是 0x81）。
 */
export const GB18030_TWO_BYTE_INDEX =
...
/**
 * WHATWG「index gb18030 ranges」的 BMP 段，压成两个平行数组（游程起点）。
 * RANGE_POINTERS[i] 起的一段 pointer 依次映射到 RANGE_CODE_POINTS[i] 起的连续码点；
 * RANGE_CODE_POINTS[i] 为 -1 表示该段无映射。pointer ≥ 189000 由公式算出星际平面码点。
 */
export const GB18030_RANGE_POINTERS: readonly number[] = [
  ...
  819, 820, 7457, 7458, 7922, 7924, 7925, 7927, 7934, 7943, 7944, 7945, 7950, 8062, 8148, 8149,
  ...
  39116, 39265, 39394, 39417, 39418,
]

export const GB18030_RANGE_CODE_POINTS: readonly number[] = [
  ...
  970, 1026, 1104, 1106, 59335, 7744, 8209, 8215, 8218, 8222, 8231, 8241, 8244, 8246, 8252,
  ...
  9702, 9735, 9738, 9793, 9795, 11906, 11909, 11913, 11917, 11928, 11944, 11947, 11951, 11956,
  ...
  65112, 65127, 65132, 65375, 65510, -1, 65534,
]
