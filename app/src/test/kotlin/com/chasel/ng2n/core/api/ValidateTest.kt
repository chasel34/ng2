...
/**
 * 各端点形状否决器的回归 —— 与 `core/net/ComboPoisonTest` 同源的判据
 * (ADR-0002 第 1 条:能洗成 JSON 但不是这个接口的响应,不许被当成功记进组合缓存)。
 *
 * RN 版只给 `thread.php` 挂了这道闸;票 07 按票面把它推广到形状足够确定的读端点,
 * 所以这一批**没有对应的 TS 用例**,判据出处写在 `Validate.kt` 的 KDoc 里。
 */
class ValidateTest {
  ...
  /** 陌生但合法的 JSON:被限流 / 镜像域名的落地页 / 没验证过的格式档都长这样。 */
  ...
  fun `app_api——data 或 other 在场即可,读的是顶层`() {
    ...
    // 顶层既没 data 也没 other:BARE 档下这种响应过得了信封,必须靠否决器拦
    assertNotNull(
  ...
  fun `noti 与 ucp——数据都在 data 的 0 上`() {
    ...
    // 空 data 放行:那一档 RN 版单独报成「资料响应里没有用户」,判成坏组合会吞掉那句话
  ...
  fun `forum_php——数字键的结果或 __MESSAGE 提示位`() {
    ...
    // 没搜到时 data 只剩 __MESSAGE,那是正常结果不是坏组合
